...
func SendEmail(to []string, subject string, templateHTML string, bodyVar interface{}, funcMap map[string]interface{}) {
	...
	const sender string = "fotocopy@sidomuncul.co.id"
	...
	gmailAuth := smtp.PlainAuth("", mailServerUsername, mailServerPassword, "smtp.gmail.com")
	...
	err = smtp.SendMail("smtp.gmail.com:587", gmailAuth, sender, to, body.Bytes())
	if err != nil {
		log.Println(err.Error())
	}
}
